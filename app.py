...
async def handle_query(query: str):
    ...
    # Attach any internal-data charts (plot_trial_operations) as inline images. The tool
    # returns a base64 PNG (rendered with matplotlib — headless, no browser, works on
    # Render). An <img> restores reliably from the stored URL on reload, unlike cl.Plotly
    # (which came back as empty axes).
    if charts:
        import base64
        for i, b64 in enumerate(charts):
            try:
                msg.elements.append(
                    cl.Image(name=f"chart-{i}", content=base64.b64decode(b64),
                             mime="image/png", display="inline")
                )
